...
exports.schema = koishi_1.Schema.object({
    ...
    autoSpeech: koishi_1.Schema.object({
        enabled: koishi_1.Schema.boolean().default(false).description('启用 ChatLuna 对话自动转语音'),
        ...
        customPrompt: koishi_1.Schema.string()
            .role('textarea')
...
function apply(ctx, config) {
        ...
        ctx.on('chatluna/after-chat', async (...args) => {
            ...
            try {
                ...
                if (config.autoSpeech.selectorMode === 'ai_sentence') {
                    try {
                        const aiSelected = await (0, tool_1.selectSpeechSentenceByAI)(ctx, config, aiText, logger);
                ...
                if (segments.length === 0)
                    return;
                const isWeixin = (0, utils_1.isWeixinLikePlatform)(session === null || session === void 0 ? void 0 : session.platform);
                const isOneBot = (0, utils_1.isOneBotPlatform)(session === null || session === void 0 ? void 0 : session.platform);
                // OneBot/QQ 需要 SILK 格式，silk-wasm 仅支持 WAV → SILK
                const apiFormat = isOneBot ? 'wav' : undefined;
                const audioBuffers = await Promise.all(segments.map(seg => (0, api_1.generateSpeech)(ctx, config, seg, config.defaultVoice, cacheManager, apiFormat)));
                ...
                if (isWeixin) {
                    const tempAudioPath = await (0, utils_1.writeTempAudioFile)(finalBuffer, (_f = config.audioFormat) !== null && _f !== void 0 ? _f : 'mp3');
                    const audioElem = (0, utils_1.makeWeixinAudioElement)(tempAudioPath);
                    try {
                        ...
                        setTimeout(() => {
                            void (0, utils_1.removeTempFile)(tempAudioPath);
                        }, 60000);
                    }
                }
                else if (isOneBot) {
                    // OneBot/QQ: 尝试转换为 SILK 格式
                    const silkBuffer = await (0, utils_1.convertToSilk)(finalBuffer, logger);
                    let audioElem;
                    if (silkBuffer) {
                        audioElem = (0, koishi_1.h).audio(silkBuffer, 'audio/silk');
                        if (config.debug)
                            logger.info(`使用 SILK 格式发送语音，大小: ${silkBuffer.length} bytes`);
                    }
                    else {
                        audioElem = (0, utils_1.makeAudioElement)(finalBuffer, 'wav');
                        if (config.debug)
                    ...
                    const audioElem = (0, utils_1.makeAudioElement)(finalBuffer, (_g = config.audioFormat) !== null && _g !== void 0 ? _g : 'mp3');
                    if (sendMode === 'voice_only') {
                        await session.send(audioElem);
                    }
                    else if (sendMode === 'mixed') {
    ...
    if (state.minimaxVitsService) {
        ...
        state.minimaxVitsService = new service_1.MinimaxVitsService(ctx, config);
    }
    // ======================================================
    // 4. 生命周期管理
    // ======================================================
    ...
    ctx.command('minivits.test <text:text>', '测试 TTS')
        ...
        if (!buffer)
            return '失败';
        if (isWeixin) {
            const tempAudioPath = await (0, utils_1.writeTempAudioFile)(buffer, (_b = config.audioFormat) !== null && _b !== void 0 ? _b : 'mp3');
            setTimeout(() => {
                void (0, utils_1.removeTempFile)(tempAudioPath);
            }, 60000);
            return (0, utils_1.makeWeixinAudioElement)(tempAudioPath);
        }
        if (isOneBot) {
            const silkBuffer = await (0, utils_1.convertToSilk)(buffer, logger);
            if (silkBuffer) {
                return (0, koishi_1.h).audio(silkBuffer, 'audio/silk');
            }
            return (0, utils_1.makeAudioElement)(buffer, 'wav');
        }
        return (0, utils_1.makeAudioElement)(buffer, config.audioFormat || 'mp3');
    });
}
exports.default = {
    name: exports.name,
